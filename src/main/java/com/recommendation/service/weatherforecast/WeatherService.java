...
public class WeatherService {
    ...
    public static OpenWeatherServiceInterface createService() {
        if(retrofit == null) {
            retrofit = new Retrofit.Builder().baseUrl(weatherProps.getUrl()).addConverterFactory(GsonConverterFactory.create()).build();
        }
    ...
    public double retrieveTemperatureByCity(String city) throws IOException {
        Call<WeatherForecastResponse> call = createService().getWeatherForecastByCity(weatherProps.getAppid(), weatherProps.getUnits(), city);
    ...
    public double retrieveTemperatureByCoordinates(String lat, String lon) throws IOException {
        Call<WeatherForecastResponse> call = createService().getWeatherForecastByCoordinates(weatherProps.getAppid(), weatherProps.getUnits(), lat, lon);
        WeatherForecastResponse weatherForecast = call.execute().body();
        return weatherForecast.getMain().getTemp();
    }
}
